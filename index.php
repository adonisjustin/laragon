...
$SKIP_DIRS = [
    ...
    'storage', 'bootstrap', '.idea', '.vscode',
    'cache', 'logs', '.next', 'dist', 'build',
    '__pycache__', '.sass-cache',
];
...
function detectLanguages(string $dir): array {
    ...
    $scan = function(string $path, int $depth) use (
        &$scan, &$found, &$scanned, $maxFiles, $LANG_MAP, $SKIP_DIRS
    ) {
...
foreach ($dirs as $dir) {
    ...
    $type = 'php';
    if ($isLaravel)   $type = 'laravel';
    elseif ($isWordPress) $type = 'wordpress';
    elseif ($isNode)  $type = 'node';
...
html, body { height:100%; font-family:'Segoe UI Variable','Segoe UI',system-ui,sans-serif; background:var(--bg-base); color:var(--t1); }
...
.topnav {
    ...
    padding:0 20px 0 16px; z-index:100;
}
.brand { display:flex; align-items:center; gap:10px; text-decoration:none; }
.brand-icon { width:28px; height:28px; flex-shrink:0; }
.brand-name { font-size:14px; font-weight:600; color:var(--t1); letter-spacing:.01em; }
.brand-name span { color:var(--accent-lit); }

.topnav-search {
    display:flex; align-items:center; gap:8px;
    background:var(--bg-input); border:1px solid var(--bd2);
    border-radius:var(--radius); padding:6px 12px;
    margin-left:20px; width:260px;
    transition:border-color var(--tr), box-shadow var(--tr);
}
.topnav-search:focus-within { border-color:var(--accent); box-shadow:0 0 0 2px rgba(0,120,212,.22); }
.topnav-search svg { width:13px; height:13px; flex-shrink:0; opacity:.4; }
.topnav-search input { background:transparent; border:none; outline:none; color:var(--t1); font:13px inherit; width:100%; }
.topnav-search input::placeholder { color:var(--t3); }
...
.nav-icon-btn {
    ...
    color:var(--t2); cursor:pointer; transition:background var(--tr), color var(--tr);
}
.nav-icon-btn:hover { background:rgba(255,255,255,.07); color:var(--t1); }
...
.vt-btn {
    ...
    border:none; background:transparent; color:var(--t3); cursor:pointer; transition:all var(--tr);
}
.vt-btn.active { background:var(--accent); color:#fff; }
.vt-btn:hover:not(.active) { background:rgba(255,255,255,.07); color:var(--t1); }
.vt-btn svg { width:14px; height:14px; fill:currentColor; }

/* ── LAYOUT ───────────────────────────────────── */
.layout { display:flex; min-height:100vh; padding-top:var(--top-h); }

/* ── SIDEBAR ──────────────────────────────────── */
.sidebar {
    width:var(--nav-w); flex-shrink:0;
    background:var(--bg-nav); border-right:1px solid var(--bd);
    position:fixed; top:var(--top-h); left:0; bottom:var(--sb-h);
    overflow-y:auto; padding:16px 0 24px; z-index:50;
}
.nav-section {
    font-size:10.5px; font-weight:700; letter-spacing:.07em;
    text-transform:uppercase; color:var(--t3); padding:14px 20px 5px;
}
...
.nav-link:hover { background:rgba(255,255,255,.06); color:var(--t1); }
.nav-link.active {
    background:rgba(0,120,212,.16); border-color:rgba(0,120,212,.22); color:var(--accent-lit);
}
.nav-link.active::before {
    content:''; position:absolute; left:-1px; top:50%; transform:translateY(-50%);
    width:3px; height:18px; background:var(--accent); border-radius:0 2px 2px 0;
}
.nav-link svg { width:15px; height:15px; flex-shrink:0; opacity:.65; }
.nav-link.active svg { opacity:1; }
.nav-pill {
    margin-left:auto; background:rgba(255,255,255,.08); color:var(--t3);
    font-size:10.5px; font-weight:600; padding:1px 8px;
    border-radius:20px; min-width:22px; text-align:center;
}
.nav-link.active .nav-pill { background:rgba(0,120,212,.28); color:var(--accent-lit); }
.nav-divider { height:1px; background:var(--bd); margin:12px 16px; }

/* ── MAIN ─────────────────────────────────────── */
.main { flex:1; margin-left:var(--nav-w); min-width:0; padding-bottom:var(--sb-h); }

.page-head {
    padding:28px 28px 0;
    display:flex; align-items:flex-end; justify-content:space-between; gap:16px;
}
.page-title { font-size:22px; font-weight:600; color:var(--t1); letter-spacing:-.01em; }
.page-subtitle { font-size:13px; color:var(--t2); margin-top:4px; }
...
.btn-accent { background:var(--accent); color:#fff; border-color:rgba(255,255,255,.08); }
.btn-accent:hover { background:var(--accent-hov); }
.btn-subtle { background:rgba(255,255,255,.06); color:var(--t2); border-color:var(--bd2); }
.btn-subtle:hover { background:rgba(255,255,255,.1); color:var(--t1); }
...
.stat-chip {
    ...
    border-radius:var(--radius); padding:10px 16px; flex:1; min-width:100px;
}
.stat-dot { width:8px; height:8px; border-radius:50%; flex-shrink:0; }
.stat-dot.all     { background:var(--accent-lit); }
.stat-dot.laravel { background:var(--c-laravel); }
.stat-dot.wp      { background:var(--c-wp); }
.stat-dot.node    { background:var(--c-node); }
.stat-dot.php     { background:var(--c-php); }
.stat-num { font-size:20px; font-weight:700; color:var(--t1); line-height:1; }
.stat-label { font-size:11px; color:var(--t3); margin-top:2px; }

/* Filter bar */
.filter-bar {
    display:flex; align-items:center; gap:6px; padding:20px 28px 0;
}
.bc { display:flex; align-items:center; gap:5px; font-size:12px; color:var(--t3); margin-right:auto; }
.bc a { color:var(--accent-lit); text-decoration:none; }
.bc a:hover { text-decoration:underline; }
.bc-sep { font-size:10px; }
.bc-cur { color:var(--t2); }

/* ── PROJECTS AREA ────────────────────────────── */
.projects-wrap { padding:16px 28px 32px; }
...
.g-card::before {
    ...
    width:3px; opacity:0; transition:opacity .2s;
    border-radius:8px 0 0 8px;
}
.g-card[data-type="laravel"]::before   { background:var(--c-laravel); }
.g-card[data-type="wordpress"]::before { background:var(--c-wp); }
.g-card[data-type="node"]::before      { background:var(--c-node); }
.g-card[data-type="php"]::before       { background:var(--c-php); }
.g-card:hover { background:var(--bg-card-hov); border-color:var(--bd2); transform:translateY(-2px); }
...
.gc-icon.laravel   { background:rgba(255,107,99,.14); }
.gc-icon.wordpress { background:rgba(77,182,224,.14); }
.gc-icon.node      { background:rgba(104,160,99,.16); }
.gc-icon.php       { background:rgba(159,163,217,.14); }
.gc-meta { flex:1; min-width:0; }
.gc-name {
    font-size:14px; font-weight:600; color:var(--t1);
    white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-bottom:3px;
}
.gc-domain { font-family:'Cascadia Code','Consolas',monospace; font-size:11px; color:var(--t3); }
...
.badge-laravel   { background:rgba(255,107,99,.14); color:var(--c-laravel); border:1px solid rgba(255,107,99,.28); }
.badge-wordpress { background:rgba(77,182,224,.14);  color:var(--c-wp);      border:1px solid rgba(77,182,224,.28); }
.badge-node      { background:rgba(104,160,99,.16);  color:var(--c-node);    border:1px solid rgba(104,160,99,.28); }
.badge-php       { background:rgba(159,163,217,.14); color:var(--c-php);     border:1px solid rgba(159,163,217,.28); }

.gc-divider { border:none; border-top:1px solid var(--bd); margin:12px 0; }

/* Info chips row */
.gc-chips { display:flex; flex-wrap:wrap; gap:6px; margin-bottom:10px; min-height:16px; }
.chip { font-size:11px; color:var(--t3); display:flex; align-items:center; gap:3px; }

/* Language pills */
.gc-langs { display:flex; flex-wrap:wrap; gap:5px; margin-bottom:12px; min-height:18px; }
.lang-pill {
...
.gc-btn {
    ...
    transition:all var(--tr); border:1px solid transparent; display:inline-block;
}
.gc-btn-p { background:var(--accent); color:#fff; border-color:rgba(255,255,255,.07); }
.gc-btn-p:hover { background:var(--accent-hov); }
.gc-btn-s { background:rgba(255,255,255,.06); color:var(--t2); border-color:var(--bd2); cursor:pointer; }
.gc-btn-s:hover { background:rgba(255,255,255,.1); color:var(--t1); }
...
.lv-row:hover { background:rgba(255,255,255,.04); }
...
.lv-icon.laravel   { background:rgba(255,107,99,.13); }
.lv-icon.wordpress { background:rgba(77,182,224,.13); }
.lv-icon.node      { background:rgba(104,160,99,.13); }
.lv-icon.php       { background:rgba(159,163,217,.13); }

.col-name   { flex:1; font-size:13px; color:var(--t1); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; min-width:0; }
.col-domain { width:190px; font-family:'Cascadia Code','Consolas',monospace; font-size:11px; color:var(--t3); flex-shrink:0; }
.col-langs  { width:200px; flex-shrink:0; display:flex; flex-wrap:wrap; gap:4px; }
.col-mod    { width:100px; font-size:11px; color:var(--t3); flex-shrink:0; }
.col-type   { width:100px; flex-shrink:0; }
.col-act    { width:110px; flex-shrink:0; display:flex; gap:5px; justify-content:flex-end; }

.lv-btn {
    padding:4px 11px; border-radius:4px;
    font:11.5px 'Segoe UI Variable','Segoe UI',system-ui,sans-serif;
    font-weight:500; cursor:pointer; text-decoration:none;
    transition:all var(--tr); border:1px solid transparent; display:inline-block; text-align:center;
}
.lv-btn-p { background:var(--accent); color:#fff; }
.lv-btn-p:hover { background:var(--accent-hov); }
.lv-btn-s { background:rgba(255,255,255,.07); color:var(--t2); border-color:var(--bd2); cursor:pointer; }
.lv-btn-s:hover { background:rgba(255,255,255,.12); color:var(--t1); }
...
.empty-state svg { width:52px; height:52px; stroke:var(--t3); margin-bottom:16px; }
...
.status-bar {
    ...
    height:var(--sb-h); background:rgba(22,22,22,.9);
    backdrop-filter:blur(10px); border-top:1px solid var(--bd);
    display:flex; align-items:center; gap:12px;
    padding:0 20px; font-size:11px; color:var(--t3); z-index:50;
}
.sb-sep  { width:1px; height:12px; background:var(--bd2); }
.sb-live { width:7px; height:7px; border-radius:50%; background:#16c60c; flex-shrink:0; }
...
.toast {
    ...
    background:#2f2f2f; border:1px solid var(--bd2);
...
@media(max-width:900px) {
    ...
    .status-bar { left:0; }
    .col-domain,.col-mod,.col-langs { display:none; }
    ...
    <div class="topnav-actions">
        ...
        <div class="view-toggle">
            ...
            <button class="vt-btn" id="btnList" onclick="setView('list')" title="List view">
                <svg viewBox="0 0 14 14"><rect x="1" y="2" width="12" height="2" rx="1"/><rect x="1" y="6" width="12" height="2" rx="1"/><rect x="1" y="10" width="12" height="2" rx="1"/></svg>
            </button>
        </div>
    </div>
</nav>
...
<div class="layout">
    ...
    <aside class="sidebar">
        ...
        <a class="nav-link active" href="#" onclick="filterType('all',this);return false;">
            <svg viewBox="0 0 16 16" fill="currentColor"><path d="M1 2.5A1.5 1.5 0 0 1 2.5 1h11A1.5 1.5 0 0 1 15 2.5v11a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 1 13.5v-11zm1.5-.5a.5.5 0 0 0-.5.5V6h12V2.5a.5.5 0 0 0-.5-.5h-11zM2 7v6.5a.5.5 0 0 0 .5.5H7V7H2zm6 0v7h5.5a.5.5 0 0 0 .5-.5V7H8z"/></svg>
            All Projects
            <span class="nav-pill"><?php echo $total; ?></span>
        </a>
        <a class="nav-link" href="#" onclick="filterType('laravel',this);return false;">
            <svg viewBox="0 0 16 16" fill="#ff6b63"><path d="M8 .5a7.5 7.5 0 1 0 0 15A7.5 7.5 0 0 0 8 .5zm0 1.5a6 6 0 1 1 0 12A6 6 0 0 1 8 2zm-1 3L5 10l2 1 .5-2L9 7l-2-2z" opacity=".9"/></svg>
            Laravel
            <span class="nav-pill"><?php echo $laravelC; ?></span>
        </a>
        <a class="nav-link" href="#" onclick="filterType('wordpress',this);return false;">
            <svg viewBox="0 0 16 16" fill="#4db6e0"><path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zM1.5 8A6.5 6.5 0 0 1 2 5.6L4.6 13A6.5 6.5 0 0 1 1.5 8zM8 14.5a6.47 6.47 0 0 1-2-.3L8.1 8.8l2.1 5.7a6.5 6.5 0 0 1-2.2.3zm2.8-.9L13 7.1H12l-1.5 4.3L9 7.1H7.9l.2-.5L9.5 1.7A6.5 6.5 0 0 1 14.2 6h-.6l-1.3 3.7 1.3-3.7h.6a6.5 6.5 0 0 1-3.4 7.6z" opacity=".9"/></svg>
            WordPress
            <span class="nav-pill"><?php echo $wpC; ?></span>
        </a>
        <a class="nav-link" href="#" onclick="filterType('node',this);return false;">
            <svg viewBox="0 0 16 16" fill="#68a063"><path d="M8 0l7 4v8l-7 4-7-4V4z" opacity=".9"/></svg>
            Node.js
            <span class="nav-pill"><?php echo $nodeC; ?></span>
        </a>
        <a class="nav-link" href="#" onclick="filterType('php',this);return false;">
            <svg viewBox="0 0 16 16" fill="#9fa3d9"><path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm0 1.5a5.5 5.5 0 1 1 0 11A5.5 5.5 0 0 1 8 2.5zm-1.3 2.8C7 4.6 7.8 4.2 9 4.2c1 0 1.8.3 2.3 1s.8 1.6.8 2.8c0 1.2-.3 2.1-.9 2.8-.5.6-1.3 1-2.2 1-.9 0-1.6-.3-2-.8l-.5 1.8H5.1L7 6.5l-.3-.7 1-1z" opacity=".9"/></svg>
            PHP / Other
    ...
    <main class="main">
        ...
        <div class="page-head">
            <div>
                ...
                <div class="page-subtitle">C:\laragon\www — <?php echo $total; ?> project<?php echo $total !== 1 ? 's' : ''; ?> found</div>
            </div>
            <div class="page-actions">
                <button class="btn btn-subtle" onclick="toggleSort()">
                    <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><line x1="2" y1="3" x2="9" y2="3"/><line x1="2" y1="7" x2="7" y2="7"/><line x1="2" y1="11" x2="5" y2="11"/></svg>
                    Sort
                </button>
                <button class="btn btn-accent" onclick="location.reload()">
                    <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M2 7a5 5 0 1 1 .8 2.6"/><polyline points="2,4 2,7 5,7"/></svg>
                    Refresh
                </button>
            </div>
        </div>
        ...
        <div class="stats-row">
            <div class="stat-chip">
                ...
                <div><div class="stat-num"><?php echo $total; ?></div><div class="stat-label">Total</div></div>
            </div>
            <div class="stat-chip">
                <span class="stat-dot laravel"></span>
                <div><div class="stat-num"><?php echo $laravelC; ?></div><div class="stat-label">Laravel</div></div>
            </div>
            <div class="stat-chip">
                <span class="stat-dot wp"></span>
                <div><div class="stat-num"><?php echo $wpC; ?></div><div class="stat-label">WordPress</div></div>
            </div>
            <div class="stat-chip">
                <span class="stat-dot node"></span>
                <div><div class="stat-num"><?php echo $nodeC; ?></div><div class="stat-label">Node.js</div></div>
            </div>
            <div class="stat-chip">
                <span class="stat-dot php"></span>
                <div><div class="stat-num"><?php echo $phpC; ?></div><div class="stat-label">PHP/Other</div></div>
            </div>
        </div>
        ...
        <div class="projects-wrap">
            ...
            <div class="grid-view" id="gridView">
                ...
                <div class="empty-state">
                    <svg viewBox="0 0 24 24" fill="none" stroke-width="1.2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
                ...
                <div class="g-card"
                     ...
                     data-name="<?php echo strtolower($n); ?>">
                    <div class="gc-head">
                        ...
                        <span class="badge badge-<?php echo $t; ?>"><?php echo ucfirst($t === 'node' ? 'Node' : $t); ?></span>
                    </div>
                    <hr class="gc-divider">
                    ...
                    <div class="gc-langs">
                        ...
                        <span class="lang-pill">
                            <span class="lang-dot" style="background:<?php echo htmlspecialchars($lang['color']); ?>"></span>
                    ...
                    <div class="gc-actions">
                        <a href="<?php echo $u; ?>" class="gc-btn gc-btn-p" target="_blank" rel="noopener">▶ Open</a>
                        <button class="gc-btn gc-btn-s"
                                onclick="openFolder('<?php echo addslashes($ph); ?>')"
            ...
            <div class="list-view" id="listView">
                ...
                <div class="lv-row"
                    ...
                    <div class="col-act">
                        <a href="<?php echo $u; ?>" class="lv-btn lv-btn-p" target="_blank" rel="noopener">▶ Open</a>
                        <button class="lv-btn lv-btn-s"
                                onclick="openFolder('<?php echo addslashes($ph); ?>')"
...
function toggleSort() {
    ...
    showToast('Sorted ' + (sortAsc ? 'A → Z' : 'Z → A'), 'info');
}
