<?php
// ──────────────────────────────────────────────────────
//  Laragon Dev Hub  –  Windows 11 Fluent Design UI
...
foreach ($dirs as $dir) {
    ...
    $real   = realpath($dir);
    $folder = $real ? 'file:///' . str_replace('/', '\\', $real) : '#';
...
.sort-btn {
    ...
    border: 1px solid var(--bd2);
    border-radius: var(--radius);
    font: 12px 'Segoe UI Variable','Segoe UI',sans-serif;
    color: var(--t2);
    cursor: pointer;
    transition: all var(--transition);
}

.sort-btn:hover { background: rgba(255,255,255,.09); color: var(--t1); }
.sort-btn svg   { width: 12px; height: 12px; opacity: .6; }
...
.gc-icon.laravel   { background: rgba(255,107,99,.13); }
.gc-icon.wordpress { background: rgba(77,182,224,.13); }
.gc-icon.php       { background: rgba(159,163,217,.13); }
...
.badge-laravel   { background: rgba(255,107,99,.14); color: var(--c-laravel); border: 1px solid rgba(255,107,99,.28);  text-transform: uppercase;}
.badge-wordpress { background: rgba(77,182,224,.14);  color: var(--c-wp);      border: 1px solid rgba(77,182,224,.28);  text-transform: uppercase;}
.badge-php       { background: rgba(159,163,217,.14); color: var(--c-php);     border: 1px solid rgba(159,163,217,.28);  text-transform: uppercase;}
...
.gc-btn-s { background: rgba(255,255,255,.06); color: var(--t2); border-color: var(--bd2); }
...
.col-name { flex: 1; min-width: 0; font-size: 13px; color: var(--t1); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.col-domain { width: 200px; font-family: 'Cascadia Code','Consolas',monospace; font-size: 11px; color: var(--t3); flex-shrink: 0; }
.col-mod { width: 110px; font-size: 11px; color: var(--t3); flex-shrink: 0; }
.col-type { width: 110px; flex-shrink: 0; }
.col-act { width: 120px; flex-shrink: 0; display: flex; gap: 6px; justify-content: flex-end; }

.lv-btn {
    padding: 4px 12px;
    border-radius: 4px;
    font: 11.5px 'Segoe UI Variable','Segoe UI',sans-serif;
    font-weight: 500;
    cursor: pointer;
    text-decoration: none;
    transition: all var(--transition);
...
.lv-btn-s { background: rgba(255,255,255,.07); color: var(--t2); border-color: var(--bd2); }
...
@media (max-width: 600px) {
    ...
    .col-domain, .col-mod { display: none; }
...
<div class="layout">
    ...
    <aside class="sidebar">
        ...
        <a class="nav-link active" href="#" onclick="filterType('all',this);return false;">
            <svg viewBox="0 0 16 16" fill="currentColor">
                <path d="M1 2.5A1.5 1.5 0 0 1 2.5 1h11A1.5 1.5 0 0 1 15 2.5v11a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 1 13.5v-11zm1.5-.5a.5.5 0 0 0-.5.5V6h12V2.5a.5.5 0 0 0-.5-.5h-11zM2 7v6.5a.5.5 0 0 0 .5.5H7V7H2zm6 0v7h5.5a.5.5 0 0 0 .5-.5V7H8z"/>
            </svg>
            All Projects
        ...
        <a class="nav-link" href="#" onclick="filterType('laravel',this);return false;">
            <svg viewBox="0 0 16 16" fill="#ff6b63">
                <path d="M8 .5a7.5 7.5 0 1 0 0 15A7.5 7.5 0 0 0 8 .5zm0 1.5a6 6 0 1 1 0 12A6 6 0 0 1 8 2zm-1 3L5 10l2 1 .5-2L9 7l-2-2z" opacity=".9"/>
            </svg>
            Laravel
        ...
        <a class="nav-link" href="#" onclick="filterType('wordpress',this);return false;">
            <svg viewBox="0 0 16 16" fill="#4db6e0">
                <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zM1.5 8A6.5 6.5 0 0 1 2 5.6L4.6 13A6.5 6.5 0 0 1 1.5 8zM8 14.5a6.47 6.47 0 0 1-2-.3L8.1 8.8l2.1 5.7a6.5 6.5 0 0 1-2.2.3zm2.8-.9L13 7.1H12l-1.5 4.3L9 7.1H7.9l.2-.5L9.5 1.7A6.5 6.5 0 0 1 14.2 6h-.6l-1.3 3.7 1.3-3.7h.6a6.5 6.5 0 0 1-3.4 7.6z" opacity=".9"/>
            </svg>
            WordPress
        ...
        <a class="nav-link" href="#" onclick="filterType('php',this);return false;">
            <svg viewBox="0 0 16 16" fill="#9fa3d9">
                <path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm0 1.5a5.5 5.5 0 1 1 0 11A5.5 5.5 0 0 1 8 2.5zm-1.3 2.8C7 4.6 7.8 4.2 9 4.2c1 0 1.8.3 2.3 1s.8 1.6.8 2.8c0 1.2-.3 2.1-.9 2.8-.5.6-1.3 1-2.2 1-.9 0-1.6-.3-2-.8l-.5 1.8H5.1L7 6.5l-.3-.7 1-1z" opacity=".9"/>
            </svg>
            PHP / Other
        ...
        <a class="nav-link" href="http://localhost/phpmyadmin" target="_blank" rel="noopener">
            <svg viewBox="0 0 16 16" fill="currentColor" opacity=".65">
                <path d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v1H0V2zm0 4h16v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V6zm5.5 1a.5.5 0 0 0-.5.5v2a.5.5 0 0 0 .5.5H6a.5.5 0 0 0 0-1h-.5V8H6a.5.5 0 0 0 0-1H5.5z"/>
            </svg>
            phpMyAdmin
        </a>
        ...
        <a class="nav-link" href="http://localhost:8025" target="_blank" rel="noopener">
            <svg viewBox="0 0 16 16" fill="currentColor" opacity=".65">
                <path d="M.05 3.555A2 2 0 0 1 2 2h12a2 2 0 0 1 1.95 1.555L8 8.414.05 3.555zM0 4.697v7.104l5.803-3.558L0 4.697zm6.761 8.083 6.57-4.027L16 11.8V14a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.2l6.761-1.02z"/>
            </svg>
            MailHog
        </a>
        ...
        <a class="nav-link" href="http://localhost" target="_blank" rel="noopener">
            <svg viewBox="0 0 16 16" fill="currentColor" opacity=".65">
                <path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm7.5-6.923c-.67.204-1.335.82-1.887 1.855-.43.832-.711 1.87-.83 3.068h2.717V1.077zM4.047 4c.03-.077.062-.155.095-.23.636-1.428 1.52-2.234 2.458-2.5v2.73H4.047zM3.603 5a13.407 13.407 0 0 0-.093 1.5c0 .528.036 1.03.093 1.5H6.5V5H3.603zM5 8.5H3.603c.05.365.108.714.17 1.038A9.98 9.98 0 0 0 6.5 10.038V8.5H5zm.6 1.838c.154.58.342 1.083.553 1.474.22.423.467.735.726.963L4.047 12a9.41 9.41 0 0 1-1.17-2.262l2.723.6z"/>
            </svg>
            Localhost
        </a>
    </aside>
    ...
    <main class="main">
        ...
        <div class="filter-bar">
            ...
            <span id="filterCount" style="font-size:12px;color:var(--t3)">
                <?php echo $total; ?> item<?php echo $total !== 1 ? 's' : ''; ?>
            </span>
        </div>
        ...
        <div class="projects-wrap">
            ...
            <div class="grid-view" id="gridView">
                ...
                <?php foreach ($projects as $p):
                    ...
                    $f  = htmlspecialchars($p['folder']);
                    $mo = htmlspecialchars($p['modified']);
                    $ic = $p['type'] === 'laravel' ? '🔺' : ($p['type'] === 'wordpress' ? '🔵' : '🟣');
                ?>
            ...
            <div class="list-view" id="listView">
                ...
                <div class="lv-row" data-type="<?php echo $t; ?>" data-name="<?php echo strtolower($n); ?>">
                    ...
                    <div class="col-type"><span class="badge badge-<?php echo $t; ?>"><?php echo ucfirst($t); ?></span></div>
                    <div class="col-act">
                        <a href="<?php echo $u; ?>" class="lv-btn lv-btn-p" target="_blank" rel="noopener">▶ Open</a>
                        <a href="<?php echo $f; ?>" class="lv-btn lv-btn-s" title="Open folder">📁</a>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
...
<div class="status-bar">
    ...
    <?php if ($wpC > 0): ?>
    <span class="sb-sep"></span>
    <span>🔵 <?php echo $wpC; ?> WordPress</span>
    <?php endif; ?>
...
function applyFilters() {
    ...
    const gridCards = document.querySelectorAll('#gridView .g-card');
    const listRows  = document.querySelectorAll('#listView .lv-row');
    let visible = 0;

    const check = (el) => {
        const nameOk = !q || el.dataset.name.includes(q);
        const typeOk = curType === 'all' || el.dataset.type === curType;
        if (nameOk && typeOk) {
    ...
    gridCards.forEach(check);
    // list rows are separate elements, count only once
    visible = 0;
    gridCards.forEach(el => { if (!el.classList.contains('hidden')) visible++; });
    listRows.forEach(check);
    visible = 0;
    gridCards.forEach(el => { if (!el.classList.contains('hidden')) visible++; });

    const sb = document.getElementById('sbCount');
...
// ─── SORT
function toggleSort() {badge badge-php
    sortAsc = !sortAsc;

    const reorder = (parentId, selector) => {
...
// ─── Fix visible count on load
(function() {
    const total = document.querySelectorAll('#gridView .g-card').length;
    const sb = document.getElementById('sbCount');
    const fc = document.getElementById('filterCount');
